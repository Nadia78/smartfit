...
//3)============= Traitement des erreurs============================
	...
	if(isset($email)&&(filter_var($email,FILTER_VALIDATE_EMAIL)===false)){
		...
		// Je vérifie si l'email n'existe pas déjé dans la bdd
	...
	if($resultEmail['email']){
		$errors['email']="L \'email existe d�j�";
		echo $errors['email'];
	}
    ...
    elseif(strlen($password) < 6) {
      ...
      $containsSpecial= preg_match('/[^a-zA-Z\d]/', $password);
        ...
        if(!$containsLetter || !$containsDigit || !$containsSpecial) {
          $errors['password'] = "Choisissez un meilleur mot de passe avec au moins un 6 caractères avec une lettre, un chiffre et caract�re sp�cial.";
        }
    } */
